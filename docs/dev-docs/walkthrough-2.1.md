...
## 8. Integration with WMVAA Portal

To provide a seamless, unified user experience across the digital school services, we integrated **WMVAA Akademia** directly into the portal gateway at `app.wmvaa.id`:

1. **Central Portal Config (`shared/portal-config.php`)**:
   Registered the `akademia` app details inside the `$apps` array metadata, setting accent color to blue (`#3b82f6`) and pointing links to `wmvaa-akademia/`.
2. **Gateway Landing Page (`index.php`)**:
   - Added a launching button tile inside the app launcher HTML grid, utilizing the mortarboard SVG icon.
   - Updated the JavaScript `apps` registry object to handle modal quick actions and description triggers.
   - Incremented the active application counter badges inside the hero and list headers (updated from `3` to `4` active applications).
3. **Redirection htaccess (`wmvaa-akademia/.htaccess`)**:
   Confirmed that root rewrites redirect requests to `public/index.php` cleanly without exposing implementation details in the browser URL.
